...
| T034 | in_progress | Height-constrained layout fallback | Default `tap_only` rendering inside line-clamped/fixed-height containers; global internal `constrainedLayoutMode` can force ruby; per-site "expand container" mode still deferred | `npm run check`, `npm test`; Google results manual test pending |
