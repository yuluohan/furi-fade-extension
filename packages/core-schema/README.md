...
- `src/recordStore.js` — the Phase 3 prep contract: deterministic per-record IDs, AppState v1 → record-store snapshot migration, snapshot → AppState reconstruction, record validation, plus the draft ingest batch helpers (`createRecordBatch`, `validateRecordBatch`, `mergeRecord`, `ingestBatch`). It is intentionally not wired into the runtime storage path yet.
